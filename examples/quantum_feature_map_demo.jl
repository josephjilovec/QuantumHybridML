"""
Demo script for quantum feature maps and kernel methods.

This script demonstrates quantum feature mapping with kernel-based classification
on a synthetic dataset, including visualizations of decision boundaries.
"""
using Pkg
...
function run_quantum_feature_map_demo()
    ...
    # Visualize decision boundaries
    println("Generating decision boundary plots...")
    ...
    println("Saved combined decision boundaries to combined_decision_boundaries.png")
    
    # Compute and plot quantum state fidelities
    println("Computing quantum state fidelities...")
    fidelities = Float64[]
    for i in 1:n_samples
        try
            state = fractal_feature_map(X[:, i], nqubits, nlayers)
            state_vec = Array(state.state)
            zero_vec = Array(zero_state(nqubits).state)
            fidelity = abs2(dot(state_vec, zero_vec))
            push!(fidelities, fidelity)
        catch e
            @warn "Error computing fidelity for sample $i: $e"
            push!(fidelities, 0.0)
        end
    end
    p_fidelity = histogram(
        fidelities,
        bins=20,
        label="Fidelity",
        title="Quantum State Fidelity Distribution",
        xlabel="Fidelity",
        ylabel="Count"
    )
    savefig(p_fidelity, "fidelity_distribution.png")
    println("Saved fidelity distribution to fidelity_distribution.png")
end
